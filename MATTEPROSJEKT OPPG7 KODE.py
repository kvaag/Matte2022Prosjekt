...
Lx_fysisk = 0.20  # 20 cm
Ly_fysisk = 0.20  # 20 cm

# Temperaturer
T_ovn = 200.0   # °C randbetingelse på alle kanter
T_start = 15.0  # °C initialbetingelse metallbiten inne
...
def g(u, t):
    #Høyreside i ODE-systemet: u'(t) = alpha_vec * L * u - alpha * F
    #Ulike alpha verdier fordelt i rommet
    return alpha_vec * (L @ u - F)
...
X_full, Y_full = np.meshgrid(x_in, y_in, indexing="ij")

#definerer området hvor metallet er: fra 5 til 15cm i x og y
metall = (
    (X_full >= 0.05) & (X_full <= 0.15) &
    (Y_full >= 0.05) & (Y_full <= 0.15)
)

# Termisk diffusivitet for metall (aluminium bronze) (22.9 mm²/s = 2.29e-5 m²/s)
# Legger til termisk diffusivitet for luft
alpha_metall = 2.29e-5
alpha_luft   = 1.94e-5   # lavere verdi

#Setter opp diffusivitet avhengig av rom
alpha_mat = np.where(metall, alpha_metall, alpha_luft)
alpha_vec = alpha_mat.reshape(m*n)

# Initialtilstand: metallet starter som 15°C, luften starter som 200°C
U0 = np.where(metall, 15.0, 200.0)
...
# ============================================================
# 7) Beregn stabilt tidssteg og simuler
# ============================================================

# Stabilitetsgrense for forlengs Euler med 2D Laplace:
#Om dt er for stor blir det ustabilt med forlengs Euler,
#den må sjekkes mot den største diffusiviteten
alpha_max = max(alpha_metall, alpha_luft)

# dt < 1 / (2 * alpha * (1/h² + 1/k²))
dt_maks = 1.0 / (2 * alpha_max * (1 / h**2 + 1 / k**2))
print(f"Maks stabilt tidssteg: {dt_maks:.2f} s")

# Total simuleringstid: 5 minutter 
total_tid = 5 * 60  # sekunder

# Velg antall tidssteg slik at dt ≈ 0.9 * dt_maks
N_steg = int(total_tid / (0.9 * dt_maks)) + 1
dt_faktisk = total_tid / (N_steg - 1)

print(f"Antall tidssteg: {N_steg}")
print(f"Faktisk dt: {dt_faktisk:.2f} s")
print(f"Stabilitet OK: dt < dt_maks? {dt_faktisk < dt_maks}")
print()

# Kjør simuleringen
print("Kjører Euler-simulering...")
u_løsning, t_vektor = euler(g, u0, 0.0, total_tid, N_steg)
print(f"Ferdig! Løsningsmatrise: {u_løsning.shape}")
print()
...
for idx, t_min in enumerate(plot_tider_min):
    ...
    plt.title(f"t = {t_min} min (midt: {temp_midt:.0f}°C)", fontsize=10)
...
# ============================================================
# 10) Temperatur i midten over tid
# ============================================================

plt.figure(figsize=(10, 5))

plt.plot(t_vektor / 60, temperatur_midten, "b-", linewidth=2, label="Temperatur i midten")
plt.axhline(y=60,  color="r",      linestyle="--", linewidth=1.5, label="60°C")
plt.axhline(y=200, color="orange",  linestyle=":",  linewidth=1, alpha=0.7, label="Ovntemperatur (200°C)")

if tid_60_sekunder is not None:
    plt.plot(tid_60_sekunder / 60, 60, "ro", markersize=10, zorder=5)
    plt.axvline(x=tid_60_sekunder / 60, color="r", linestyle=":", alpha=0.5)
    plt.annotate(
        f"60°C ved t = {tid_60_sekunder/60:.1f} min",
        xy=(tid_60_sekunder / 60, 60),
        xytext=(tid_60_sekunder / 60 + 5, 45),
        fontsize=11,
...
plt.title("Temperaturutvikling i midten av metallbiten", fontsize=14, fontweight="bold")
plt.legend(fontsize=11)
plt.grid(True, alpha=0.3)
plt.xlim(0, total_tid / 60)
plt.ylim(0, 220)
plt.tight_layout()
plt.savefig("temperatur_midten.png", dpi=150, bbox_inches="tight")
plt.show()
print("Lagret: temperatur_midten.png")

# ============================================================
# 11) Varmeplot når midten akkurat når 60°C
# ============================================================
...
if tid_60_sekunder is not None:
    ...
    plt.xlabel("x (cm)")
    plt.ylabel("y (cm)")
    plt.title(f"Varmeplot når midten når 60°C (t = {tid_60_sekunder/60:.1f} min)",
...
# ============================================================
# 12) 3D-plott av løsningen ved t der midten = 60°C
# ============================================================
...
if tid_60_sekunder is not None:
    ...
    ax.plot_surface(X_full * 100, Y_full * 100, Z_60, cmap="RdYlBu_r")
    ax.set_xlabel("x (cm)")
    ax.set_ylabel("y (cm)")
    ax.set_zlabel("Temperatur (°C)")
    ax.set_title(f"3D-plott ved t = {tid_60_sekunder/60:.1f} min (midten = 60°C)")
...
print("OPPSUMMERING")
print("=" * 55)
print(f"Objekt:               Metallbit")
print(f"Termisk diffusivitet: α = {alpha_metall} m²/s for metall, for luft = {alpha_luft}m²/s")
print(f"Tverrsnitt:           {Lx_fysisk*100:.0f} cm × {Ly_fysisk*100:.0f} cm")
print(f"Ovntemperatur:        {T_ovn:.0f}°C")
print(f"Starttemperatur:      {T_start:.0f}°C")
print(f"Indre punkter:        {m} × {n}")
print(f"Tidssteg:             dt = {dt_faktisk:.2f} s")
if tid_60_sekunder is not None:
    print(f"Tid til 60°C i midten: {tid_60_sekunder:.0f} s = {tid_60_sekunder/60:.1f} min")
print(f"Temp i midten etter {total_tid/60:.0f} min: {temperatur_midten[-1]:.1f}°C")
print("=" * 55)
...
stride = 5
n_frames = 60  # som i originalkoden
